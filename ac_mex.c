 ...
 
{
    ...
    if ( (iNI==0) || (iNI%NitUpdateHr==0) )
    {
        ...
        *pfAreaOut = fNormalizationOut;
        ...
        for (ix=0; ix< iNx; ix++)
            for (iy=0; iy< iNy; iy++)
                pfHr[X(ix,iy)] = +( SQR(fMeanIn-pfIm0[X(ix,iy)]) - SQR(fMeanOut-pfIm0[X(ix,iy)]) ); /* correct the sign of the term h_r */
...
void ChanVeseSegmentationNew (double *pfIm0, int width, int height, int iNf, double *pfu, int KMax, int LMax, double fLambda, double fMu, double *pfHr, double *fMeanIn, double *fMeanOut, double *pfAreaIn, double *pfAreaOut)
{
    ...
    /* printf("matlab before cycle ==> fMeanIn %lf, fMeanOut %lf, pfAreaIn %lf, pfAreaOut  %lf\n",*fMeanIn,*fMeanOut,*pfAreaIn,*pfAreaOut); */
    ...
    /* fLambda /= fRangeHr; */ /* remove normalization of lambda with respect to
                                  the range of h_r */
...
extern void mexFunction(int iNOut, mxArray *pmxOut[], int iNIn, const mxArray *pmxIn[]) 
{
  ...
  double *pfHr; 
  double *pfImDap, *pfImRed, *pfImGreen, *pfImViol, *pfVecParameters;
  ...
  pfImGreen = mxGetData(pmxIn[1]);   /* Green Image  */
  pfImRed   = mxGetData(pmxIn[2]);   /* Red Image    */
  pfImViol   = mxGetData(pmxIn[3]);  /* Violet Image */
  pfu = mxGetData(pmxIn[4]);         /* u function minimizer */
  pfVecParameters = mxGetData(pmxIn[5]); /* Vector of AC parameters */
  ...
  /* printf("Inside MEX-function ===> Frame Number %s..\n",fstr); */
  ...
  pfunew = pfu;
  /* Mean values of objects and background regions of Dapi (blu) image */
  pfMeanIn[0] = c1;
  pfMeanOut[0] = c2;
  pfDen1[0] = den1;
  pfDen2[0] = den2;
  ...
  GetRegionsAverages(pfImGreen, pfu, width, height, pfHr, iNbItersUpdateHr, &c1, &c2, &den1, &den2, 0); 
  printf("Green Frame     ==> fMeanIn %f fMeanOut %f \n",c1,c2);
  /* Mean values of objects and background regions of Green image */
  pfMeanIn[1] = c1;
  pfMeanOut[1] = c2;
  pfDen1[1] = den1;
  pfDen2[1] = den2;

  GetRegionsAverages(pfImRed, pfu, width, height, pfHr, iNbItersUpdateHr, &c1, &c2, &den1, &den2, 0); 
  printf("Red Frame       ==> fMeanIn %f fMeanOut %f \n",c1,c2);
  /* Mean values of objects and background regions of Red image */
  pfMeanIn[2] = c1;
  pfMeanOut[2] = c2;
  pfDen1[2] = den1;
  pfDen2[2] = den2;
  GetRegionsAverages(pfImViol, pfu, width, height, pfHr, iNbItersUpdateHr, &c1, &c2, &den1, &den2, 0); 
  printf("Violet Frame    ==> fMeanIn %f fMeanOut %f \n",c1,c2);
  /* Mean values of objects and background regions of Violet image */
  pfMeanIn[3] = c1;
  pfMeanOut[3] = c2;
  pfDen1[3] = den1;
  pfDen2[3] = den2;
